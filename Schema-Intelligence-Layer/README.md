...
*   **[app/config.py](app/config.py)**: Implements settings management using Pydantic Settings. Dynamically parses environment variables from the `.env` configuration file (such as `GROQ_API_KEY`, `AZURE_OPENAI_API_KEY`/`AZURE_OPENAI_ENDPOINT`/`AZURE_OPENAI_DEPLOYMENT`, custom database paths, and API port setups) and enforces type-safety checks across them.
...
*   **[app/services/llm_service.py](app/services/llm_service.py)**: Tries Azure OpenAI first (when `AZURE_OPENAI_*` is configured in the root `.env`), then falls back to the Groq API (`llama-3.1-8b-instant` by default) — see `_call_azure_openai`/`_chat_complete`. Generates natural language explanations for columns based on names and samples, and classifies the dataset's business domain (e.g. Finance, Logistics, HR) with confidence scores and logic reasoning. Column description generation is batched — datasets wider than `MAX_COLUMNS_PER_LLM_CALL` (30) columns are split into multiple smaller LLM calls, since a single prompt covering every column in a very wide dataset can exceed Groq's per-request token limit — and those batches run **concurrently** via a bounded `ThreadPoolExecutor` (`generate_column_descriptions`, capped at 5 in flight at once), not one-at-a-time, so a 141-column dataset's 5 batches complete in roughly one batch's latency instead of five. Domain classification similarly caps the columns it includes in its prompt to the same limit, using them as a representative sample rather than requiring exhaustive coverage.
...
#### Parameters:
*   `AZURE_OPENAI_API_KEY` / `AZURE_OPENAI_ENDPOINT` / `AZURE_OPENAI_DEPLOYMENT` *(root `.env`, optional)*: when all three are set, `llm_service.py`'s `_call_azure_openai`/`_chat_complete` helpers try Azure OpenAI first for every column-description/classification call, falling back to Groq (below) only if Azure fails or isn't configured — never required, only preferred.
*   `GROQ_API_KEY` *(root `.env`)*: Your private Groq Cloud developer token (obtain one from the [Groq Console](https://console.groq.com/keys)). Used as the fallback LLM path (or the primary one, if Azure isn't configured) for description and classification tasks.
