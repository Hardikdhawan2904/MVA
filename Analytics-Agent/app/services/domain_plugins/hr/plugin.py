"""app/services/domain_plugins/hr/plugin.py — thin starter plugin for
Agent 2's "HR" domain. See finance/plugin.py's docstring for the shared
design rationale (ThinKPIDomainPlugin, nonexistent hierarchy/business_rules
paths)."""
...
_KPI_ALIASES = {
    ...
    "turnover": "attrition_rate",
    "payroll": "payroll_cost",
    "payroll cost": "payroll_cost",
}


class HRPlugin(ThinKPIDomainPlugin):
    def applies_to(self, detected_domain: str) -> bool:
        return (detected_domain or "").strip().lower() == "hr"
