...
1. Generate a candidate business rule using Groq.
...
class KnowledgeUpdateTool:
    ...
    def __init__(self, rule_engine):
        ...
        if GROQ_API_KEY:
            try:
                ...
                logger.info("KnowledgeUpdateTool: Groq client ready")
            except ImportError:
                logger.warning("groq not installed — knowledge generation disabled")
    ...
    def update(self, kpi_name: str) -> dict:
        ...
        if not candidate:
            return {
                ...
                "kpi_key": kpi_name,
                "definition": None,
                "reason": "Could not generate candidate — Groq unavailable or returned invalid JSON",
            }
    ...
    def _generate_candidate(self, kpi_name: str) -> dict | None:
        if not self._client:
            logger.warning("Groq unavailable — using minimal stub for known insurance KPIs")
