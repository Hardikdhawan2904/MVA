...
def test_static_fallback_kpi_passes_validation_end_to_end():
    """The exact no-Groq path _validate() is actually exercised through in
    production — update() with no Groq client configured. Forced
    deterministically (not relying on GROQ_API_KEY being unset in this
    environment) so this test's outcome never depends on real env state."""
