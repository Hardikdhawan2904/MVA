...
## Agent 3 — Analytics Agent (optional; pipeline-gated to Insurance today)
...
- **Runs only when**: `primary_domain == "Insurance"`, the upload is a `.csv`, and `business_question` was supplied. Otherwise `agent3` in the response is `{"status": "skipped", "reason": "..."}`. This gate is the Orchestrator's own, unchanged by Agent 3's redesign — Agent 3 itself no longer needs it (a direct `POST /analyze` call works against any dataset today), but the Orchestrator hasn't been updated to route non-Insurance domains to it yet.
